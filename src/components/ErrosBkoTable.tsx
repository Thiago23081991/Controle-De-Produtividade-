...
export const ErrosBkoTable: React.FC = () => {
    const { erros, isLoading, deleteErro } = useErrosBko();
    ...
    const handleDelete = async (id: string, expertName: string) => {
        if (!confirm(`Remover registro de erro BKO de ${expertName}?`)) return;
        await deleteErro(id);
    };
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-black text-slate-800 dark:text-white text-base tracking-tight">
                        {isAdmin ? 'Registros de Erros BKO' : 'Meus Registros de Erros BKO'}
                    </h3>
            ...
            {isLoading ? (
                ...
                <div className="py-16 text-center">
                    ...
                    <p className="text-slate-400 font-bold">Nenhum erro BKO registrado no período</p>
                </div>
            ) : (
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                            {visibleErros.map((erro, idx) => (
                                <tr key={erro.id || idx} className="hover:bg-indigo-50/40 dark:hover:bg-indigo-900/10 transition-colors group">
                                    ...
                                    <td className="px-5 py-4">
                                        <span className="inline-flex items-center gap-1.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 px-3 py-1.5 rounded-lg text-xs font-black tracking-wide">
                                    ...
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-[10px] font-black text-indigo-600 dark:text-indigo-300 shrink-0">
                                    ...
                                    <td className="px-5 py-4 whitespace-nowrap">
                                        <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                                            erro.motivo?.startsWith('Erro Script')
                                    ...
                                    {isAdmin && (
                                        <td className="px-5 py-4 text-center">
                                            <button
                                                ...
                                                className="opacity-0 group-hover:opacity-100 p-2 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-400 hover:text-indigo-600 transition-all"
