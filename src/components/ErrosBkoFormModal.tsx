...
interface ErrosBkoFormModalProps {
    isOpen: boolean;
    onClose: () => void;
}
...
const MOTIVO_OPTIONS = [
    ...
    'Diferença de tonalidade sem comparativo',
...
export const ErrosBkoFormModal: React.FC<ErrosBkoFormModalProps> = ({ isOpen, onClose }) => {
    const { addErro, isSaving } = useErrosBko();
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300">
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-slate-100 dark:border-slate-800 overflow-hidden animate-in zoom-in duration-300">
                ...
                {/* Header */}
                <div className="bg-gradient-to-br from-indigo-600 to-violet-700 p-8 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between relative z-10">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-indigo-200 text-[10px] font-bold uppercase tracking-widest">BKO</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-3 px-4 text-sm font-bold text-slate-700 dark:text-slate-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-all"
                        />
                    </div>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            Nº do Caso / Atividade <span className="text-indigo-500">*</span>
                        </label>
                        <input
                            type="text"
                            value={numeroCaso}
                            ...
                            required
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-3 px-4 text-sm font-bold text-slate-700 dark:text-slate-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-all placeholder:font-normal placeholder:text-slate-300"
                        />
                    </div>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            Expert que Errou <span className="text-indigo-500">*</span>
                        </label>
                        <select
                            value={expertName}
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            Motivo <span className="text-indigo-500">*</span>
                        </label>
                        <select
                            value={motivo}
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            Onde está o erro no script <span className="text-indigo-500">*</span>
                        </label>
                        <textarea
                            value={descricaoErro}
                            onChange={e => setDescricaoErro(e.target.value)}
                            placeholder="Descreva onde e qual foi o erro no script..."
                            required
                            rows={4}
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-3 px-4 text-sm font-bold text-slate-700 dark:text-slate-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-all resize-none placeholder:font-normal placeholder:text-slate-300"
                        />
                    </div>
